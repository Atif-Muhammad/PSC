...
export class AuthService {
    constructor(private prisma: PrismaService, private jwtService: JwtService, private mailer: MailerService) { }

    async generateTokens(payload: { id: number | string, name: string, email: string, role?: string, status?: string }) {
    ...
    async refreshTokens(payload: { id: number | string, name: string, email: string, role?: string, status?: string }) {
    ...
    async removeAdmin(adminID: number) {
        const existingAdmin = await this.prisma.admin.findUnique({ where: { id: adminID } });
        if (!existingAdmin) {
    ...
    async getMember(memberID: string){
        const member = await this.prisma.member.findUnique({where: {Membership_No: String(memberID)}})
         if (!member) {
    ...
    async storeOTP(memberID: string, otp: number){
        const otpSaved = await this.prisma.member.update({where: {Membership_No: String(memberID)}, data: {otp}})
        if(!otpSaved) {
            throw new HttpException("Unknow Error while saving otp for member", HttpStatus.SERVICE_UNAVAILABLE);
        }
        return otpSaved
    }
